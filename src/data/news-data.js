// Noticias de la clínica (contenido estático)
export const NEWS_ARTICLES = [
  {
    id: 'Convocatoria Asociacion de Usuarios',
    title: 'Convoca a la ciudadania a conformar parte de la Asociación de Usuarios',
    summary: 'Invitamos a todos los usuarios a participar en la conformación de nuestra Asociación de Usuarios para fortalecer la comunicación y mejorar nuestros servicios.',
    content: `
La Clínica Norte Especialidades SAS se complace en anunciar la convocatoria a la Asociación de Usuarios, un espacio de participación y diálogo entre la comunidad y nuestros profesionales de la salud. Esta iniciativa busca:

- Fortalecer la comunicación con nuestros usuarios
- Recoger sugerencias y necesidades de la comunidad
- Mejorar la calidad de nuestros servicios

Invitamos a todos los interesados a inscribirse y ser parte activa en cada etapa de la Clínica Norte Especialidades.
Fecha de inscripción: 4 de octubre de 2025 a las 3 PM.
    `,
    date: '2025-09-25',
    category: 'Institucional',
    image: '/images/news/2025/septiembre/convocatoria_asociacion_usuarios_2025.jpg',
    author: 'Gerencia General',
    featured: true
  },
  // Example external link for the first article (opens in new tab from modal)
  {
    id: 'convocatoria-enlace',
    title: 'Convocatoria - más información en PDF',
    summary: 'Más detalles y formulario disponible en un documento PDF externo.',
    content: `Consulta el PDF con la información completa y el formulario de inscripción.`,
    date: '2024-01-25',
    category: 'Institucional',
    image: '/images/news/2025/septiembre/convocatoria_asociacion_usuarios_2025.jpg',
    author: 'Gerencia General',
    featured: false,
    externalLink: 'https://example.com/convocatoria-inscripcion.pdf',
    videoUrl: 'https://www.youtube.com/watch?v=cn-eX18i12c&list=RDcn-eX18i12c&start_radio=1'
  },
  {
    id: 'jornada-vacunacion',
    title: 'Jornada de vacunación contra la influenza',
    summary: 'Durante el mes de febrero realizaremos jornadas especiales de vacunación para toda la familia.',
    content: `
Como parte de nuestro compromiso con la medicina preventiva, la Clínica Norte Especialidades SAS realizará jornadas especiales de vacunación contra la influenza durante todo el mes de febrero.

**Fechas y horarios:**
- Todos los sábados de febrero
- Horario: 8:00 AM a 2:00 PM
- Sin cita previa

**Incluye:**
- Vacuna contra influenza estacional
- Evaluación médica previa
- Tarjeta de vacunación actualizada

La vacunación es especialmente importante para adultos mayores, niños, mujeres embarazadas y personas con enfermedades crónicas.
    `,
    date: '2024-01-30',
    category: 'Prevención',
    image: '/images/news/2025/septiembre/nuevo-servicio-cardiologia-especializada.jpg',
    author: 'Dr. Carlos Rodríguez',
    featured: false
  },
  {
    id: 'convenio-eps',
    title: 'Nuevos convenios con EPS principales',
    summary: 'Ampliamos nuestra red de convenios para facilitar el acceso a nuestros servicios médicos.',
    content: `
La Clínica Norte Especialidades SAS anuncia la firma de nuevos convenios con las principales EPS del país, facilitando el acceso a nuestros servicios para un mayor número de pacientes.

**Nuevos convenios:**
- Sanitas EPS
- Sura EPS
- Compensar EPS
- Famisanar EPS

**Servicios cubiertos:**
- Consulta médica general
- Especialidades médicas
- Exámenes de laboratorio
- Procedimientos menores
- Imagenología básica

Estos convenios entran en vigencia a partir del 1 de marzo de 2024.
    `,
    date: '2024-02-10',
    category: 'Convenios',
    image: '/images/news/2025/septiembre/jornada-salud-preventiva-comunidad.jpg',
    author: 'Área Administrativa',
    featured: true
  },
  {
    id: 'telemedicina',
    title: 'Lanzamiento del servicio de telemedicina',
    summary: 'Ahora puedes tener consultas médicas desde la comodidad de tu hogar a través de nuestro servicio de telemedicina.',
    content: `
La Clínica Norte Especialidades SAS se adapta a las nuevas tecnologías e inaugura su servicio de telemedicina, permitiendo consultas médicas virtuales para mayor comodidad de nuestros pacientes.

**Características del servicio:**
- Consultas por videollamada
- Plataforma segura y confiable
- Horarios flexibles
- Prescripción médica digital
- Seguimiento de tratamientos

**Especialidades disponibles:**
- Medicina general
- Control de enfermedades crónicas
- Consulta de resultados
- Orientación médica

Para acceder al servicio, agenda tu cita a través de nuestra página web o línea telefónica.
    `,
    date: '2024-02-20',
    category: 'Tecnología',
    image: '/images/news/2025/septiembre/ampliacion-horarios-atencion-medica.jpg',
    author: 'Departamento de Sistemas',
    featured: false
  }
];

// Categorías de noticias
export const NEWS_CATEGORIES = [
  'Todas',
  'Institucional',
  'Prevención',
  'Convenios',
  'Tecnología',
  'Eventos',
  'Salud'
];
